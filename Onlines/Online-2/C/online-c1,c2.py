...
class DiscreteSignal:
    ...
    def plot(self, figsize=(8, 3),y_range=(-1, 4), title=None, x_label='n (Time Index)', y_label='x[n]', saveTo=None):
        ...
        # set y-axis range
        y_range = (min(np.min(self.values),y_range[0])-1, max(np.max(self.values), y_range[1]) + 1)
        plt.ylim(*y_range)
        # plot the signal (X,Y)
...
def plot_impulses(unit_impulses, sum_of_unit_impulses, suptitle, which_impulse,y_range, plot_name="Impulses"):
    ...
    sum_of_unit_impulses.plot_ax(ax=axs[-1], title="Sum", y_range=y_range)
...
def show_linear(lti, input_signal,y_range=(-1,3)):
    ...
    unit_impulses = []
    sum_of_unit_impulses = DiscreteSignal(input_signal.INF)
    ...
    plot_impulses(unit_impulses, sum_of_unit_impulses, suptitle="Impulses multiplied by coefficients",
                  which_impulse="input impulse", plot_name="Impulses",y_range=y_range)


def show_output(lti, input_signal,y_range=(-1,3)):
    ...
    plot_impulses(impulse_responses, output, suptitle="Response of Input Signal", which_impulse="impulse response",
                  plot_name="Impulse_Response",y_range=y_range)
...
def main():
    ...
    for (i, coeff) in enumerate(poly1):
        impulse_response.set_value_at_time(i, coeff)

    # impulse_response.plot(y_range=(-4,4),title=f"Figure 1: Impulse Response of the system,INF= {impulse_response.INF}")
    input_signal = DiscreteSignal(INF)
    for (i, coeff) in enumerate(poly2):
        input_signal.set_value_at_time(i, coeff)

    # input_signal.plot(y_range=(-4,4),title=f"Figure 2: Input Signal,INF= {input_signal.INF}")

    lti = LtiDiscrete(impulse_response)
    out,__,__ = lti.output(input_signal)
    esm = out.getValues()[INF:INF+len(poly1)+len(poly2)-1]

    print("The output signal is: ", esm)

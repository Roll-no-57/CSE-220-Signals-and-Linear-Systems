...
def main():
    ...
    # n = 3
    # alpha = 0.8
    alpaha_prime = 1 - alpha
    # impluse response of the system
    input_signal = DiscreteSignal(INF)
    for i,price in enumerate(price_list):
        input_signal.set_value_at_time(i, price)

    input_signal.plot(title=f"Figure 1: Impulse Response of the system,INF= {input_signal.INF}")

    impulse_response = DiscreteSignal(INF)
    for i in range(n):
        impulse_response.set_value_at_time(i, alpha * (alpaha_prime ** i))

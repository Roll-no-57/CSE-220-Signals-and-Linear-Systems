...
def main():
    ...
    # input_signal.set_value_at_time(0, .5)
    for i in range(n):
        impulse_response.set_value_at_time(i,(alpha_prime**i)*alpha)
        
    impulse_response.plot(title=f"Figure 2: Input Signal,INF= {impulse_response.INF}")
    ...
    show_linear(lti, input_signal)
    # shows the output and the impulse responses of the system
    out=show_output(lti, input_signal)
